...
def build_common_co_multiprocess(cluster_lst: List[Cluster], new_clusters: List[Cluster], \
    ...
    try:
        cluster_hashes, split_index = UnevenNestedList.init_combined(cluster_lst, new_clusters)
        
        if len(cluster_lst) > 0:
            parameters = ( (split_index + i, j) for i in 
                          tqdm(range(len(new_clusters)), total=len(new_clusters), desc='adding clusters to CCO P1')\
                              for j in range(len(cluster_lst)))
            
            optimized_chunksize = calc_optimal_chunksize(len(new_clusters)*len(cluster_lst), chunksize)
            #go through all new clusters and see if they are related to existing clusters
            with mp.Pool(mp.cpu_count(), initializer=init_mp, initargs=(shared_co_dct, cluster_hashes), maxtasksperchild=optimized_chunksize) as p:
                for id1, id2, value in p.imap_unordered(__partial_common_co__, iterable=parameters, chunksize=optimized_chunksize):
                    cluster1, cluster2 = new_clusters[id1 - split_index], cluster_lst[id2]
        ...
        parameters = ((split_index + i, split_index + j) for i in tqdm(range(len(new_clusters)), 'adding clusters to CCO P2')\
                            for j in range(i+1, len(new_clusters)))
        
        optimized_chunksize = calc_optimal_chunksize(len(new_clusters)*1.25, chunksize)
        with mp.Pool(mp.cpu_count(), initializer=init_mp, initargs=(shared_co_dct, cluster_hashes), maxtasksperchild=optimized_chunksize) as p:
            for id1, id2, value in p.imap_unordered(__partial_common_co__, iterable=parameters, chunksize=optimized_chunksize):
                cluster1, cluster2 = new_clusters[id1 - split_index], new_clusters[id2 - split_index]
                matrix[cluster1, cluster2] = value
        # print(sys.getsizeof(matrix.__internal__) )
        # print(sys.getsizeof(new_hashes))
        return matrix
    except IndexError as ex:
        print(id1, id2, len(new_clusters), split_index, ex)
    finally:
        global shared_hash_lst
        shared_hash_lst = None
