...
NEWFILE = os.path.join(os.getcwd(), 'new_cluster_file.temp')
OLDFILE = os.path.join(os.getcwd(), 'cahced_cluster_file.temp')
NEWSIZE = 0
OLDSIZE = 0
...
def build_common_co_multiprocess(cluster_lst: List[Cluster], new_clusters: List[Cluster], \
    ...
    try:
        new_hashes = UnevenNestedList.init_memmap(NEWFILE, new_clusters)
        save_array(new_hashes, True)
        ...
        if len(cluster_lst) > 0:
            ...
            old_hashes = UnevenNestedList.init_memmap(OLDFILE, cluster_lst)
            save_array(old_hashes, False)
            del old_hashes
            
            
            #go through all new clusters and see if they are related to existing clusters
            with mp.Pool(mp.cpu_count(), initializer=init_mp, initargs=(shared_co_dct, None), maxtasksperchild=10) as p:
...
def save_array(arr: UnevenNestedList, is_new: bool):
    if is_new:
        global NEW_KEYS, NEWSIZE
        NEW_KEYS = arr._starts
        NEWSIZE = len(arr._store)
    else:
        global OLD_KEYS, OLDSIZE
        OLD_KEYS = arr._starts
        OLDSIZE = len(arr._store)
        

def clean_tmp_files(is_new):
    file = None
    if is_new:
        global NEW_KEYS, NEWSIZE, NEWFILE
        file = NEWFILE
        NEWSIZE = 0
        NEW_KEYS = None
    else:
        global OLD_KEYS, OLDSIZE, OLDFILE
        file = OLDFILE
        OLDSIZE = 0
        OLD_KEYS = None
...
def load_hashlst(keys, filename, size) -> UnevenNestedList:
    values = np.memmap(filename=filename, mode='r', dtype=np.longlong, shape=size)
    return UnevenNestedList(keys, values)
...
def __partial_shared_oldnew_common_co_entry(tuple: Tuple[object, List[int]]):
    ...
    global shared_co_dct, OLD_KEYS, OLDFILE, OLDSIZE
    co_matrix = shared_co_dct
    hash_lst = load_hashlst(OLD_KEYS, OLDFILE, OLDSIZE)
    
    try:
        for index, cluster2 in enumerate(hash_lst):
...
def __partial_shared_newnew_common_co_entry(tuple: int):
    ...
    global shared_co_dct, NEW_KEYS, NEWFILE, NEWSIZE
    if len(NEW_KEYS) <= index+1: return (id, result)
    
    co_matrix = shared_co_dct
    hash_lst = load_hashlst(NEW_KEYS, NEWFILE, NEWSIZE)
    
    try:
        hash_cluster: List[int] = hash_lst[index]
        if len(hash_cluster) == 0: return (id, [])
        for index in range(index + 1, len(hash_lst)):
            cluster2 = hash_lst[index]
            if len(cluster2) == 0: continue
