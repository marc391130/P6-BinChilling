...
public sealed class BinRefiner2
{
    ...
    public BinRefiner2(double minCoValue, IReadOnlyDictionary<CoTuple, double> coMatrix, BinEvaluator evaluator)
    {
    ...
    {
        if(progressBar.CurrentTick >= progressBar.MaxTicks) return TimeSpan.Zero;
    ...
    private RefineResult RefineClustersMultiThreaded_NoBreak_NoRepeat(
        IEnumerable<KeyValuePair<string[], IEnumerable<string[]>>> clusterList, int size, ISet<int> skipSet)
    {
        using var progressBar = CreateProgressBar(size);
