...
class AdaptiveClusterEnsembler2(Ensembler):
    def __init__(self, initial_alpha1_thredshold: float = 0.8, initial_delta_aplha: float = 0.1,\
        alpha1_min: float = 0.6, alpha2: float = 0.2, logger: Logger = None, quality_measurer: QualityMeasuerer = None):
        ...
        self.logger = logger if logger is not None else Logger(console_log=True)
    ...
    def ensemble(self, gamma: PartitionSet) -> Partition:
        ...
        target_clusters = int(max([len(partition) for partition in gamma]))
        ...
        print(f"Found {len(certain_clusters)} clusters with certain objects")
        ...
        return self.assign_item_to_one_cluster(gamma, candidate_clusters, non_candidate_clusters, similarity_matrix, candidate_memberMatrix, non_candidate_memberMatrix)
    ...
    def assign_item_to_one_cluster(self, gamma: PartitionSet, candidate_clusters: List[Cluster], non_candidate_clusters: List[Cluster],\
        ...
        print("\nFound: ")
        print(f"Totally certain objects: {len(totally_certain_lst)}")
        print(f"Certain objects: {len(certain_lst)}")
        print(f"Uncertain objects: {len(uncertain_lst)}")
        print(f"Totally uncertain objects: {len(totally_uncertain_lst)}")
        print('\n')
    ...
    def reidentify_totally_uncertain_item(self, totally_uncertain_item_lst: List, candidate_clusters: List[Cluster],\
        ...
        self.log("reidentified totally uncertain objects to the following certainty:")
        self.log(f"Totally certain objects: {len(tot_certain)}")
        self.log(f"Certain objects: {len(certain)}")
        self.log(f"Uncertain objects: {len(uncertain)}")
...
def __naive_partial_cluster_certainty_degree__(\
    tuple : Tuple[int, np.matrix] ) -> Tuple[int, float, int]:
    # item, id, cluster_dct, ensembler = tuple
    item_index, matrix = tuple
    ...
    for c_index in range(len(matrix[item_index])):
        value = matrix[item_index, c_index]
        if value > 0:
            count +=1
    ...
    if max_value == 1:
        print(count)
    return (item_index, max_value, max_index)
