...
""".format(len(self.matrix), self.nclusters, self.CUDA, self.MAXSTEPS, self.MINSUCCESSES,
    ...
    def __init__(self, matrix, maxsteps=25, windowsize=200, minsuccesses=20, destroy=False,
                normalized=False, cuda=False, randomseed = 0):
        ...
        _np.random.RandomState(randomseed).shuffle(matrix)
        indices = _np.random.RandomState(randomseed).permutation(len(matrix))
        ...
        self.CUDA = cuda
        self.RNG = _random.Random(randomseed)
...
def cluster(matrix, randomseed=0, labels=None, maxsteps=25, windowsize=200, minsuccesses=20, destroy=False,
    ...
    it = ClusterGenerator(matrix, maxsteps, windowsize, minsuccesses, destroy, normalized, cuda, randomseed)
    for cluster in it:
