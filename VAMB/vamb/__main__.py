...
def cluster(outdir, latent, contignames, windowsize, minsuccesses, maxclusters,
            minclustersize, separator, cuda, logfile, randomseed, result_amount):
    begintime = time.time()
    ...
    for i in range(result_amount):
        ...
        filepath = os.path.join(outdir, f'clusters{i}.tsv')
        with open(filepath, 'w') as clustersfile:
...
def run(outdir, fastapath, tnfpath, namespath, lengthspath, bampaths, rpkmpath, jgipath,
        ...
        minsuccesses, minclustersize, separator, maxclusters, minfasta, logfile, randomseed, result_amount):
    ...
    cluster(outdir, latent, contignames, windowsize, minsuccesses, maxclusters,
            minclustersize, separator, cuda, logfile, randomseed=randomseed, result_amount=result_amount)
...
def main():
    ...
    inputos.add_argument('-y', dest='loops', metavar='', type=int, default=1,
                         help='number of clustering results. Each with unique seed [default = 1]')
    ...
    clusto.add_argument('-rand', dest='randomseed', metavar='', type=int,
                        default=0, help='seed for random, this is counted up by the -y argument [default is 0]')
    ...
    if args.loops < 1:
        raise argparse.ArgumentTypeError('less than 1 results is requested')
